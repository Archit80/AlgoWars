...
export default function WaitingRoom() {
  ...
  // Demo: mock user info (replace with real user context)
  const user = { name: "Archit", level: 24 };
  ...
  const shareLink = typeof window !== 'undefined' && match?.roomCode ? `${window.location.origin}/battle/join?code=${match.roomCode}` : "";
  ...
  return (
    <div className="min-h-screen flex items-start justify-center bg-[#0E0E0E] text-white">
      ...
      <div className="max-w-2xl w-full mt-12">
        <h1 className={`${spaceGrotesk.className} text-4xl font-bold text-center mb-2`}>Battle Lobby</h1>
        <div className="text-center text-neutral-400 mb-8 text-lg">{match?.status === "WAITING" ? "Waiting for your friend to join..." : ""}</div>
        <div className="rounded-2xl shadow-sm bg-neutral-900 border border-neutral-800 p-8 mx-auto relative shadow-black hover:shadow-none ">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className={`text-xl font-bold ${spaceGrotesk.className} text-white`}>Room: <span className="text-lime-400">{match?.roomCode || "-"}</span></div>
            {match?.isPrivate && (
              <Button
                onClick={() => {
              ...
              >
                {copied ? <Check size={18} className="text-lime-400" /> : <Copy size={18} className="text-lime-400" />}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(match?.categories || []).map((cat, i) => (
                  <span key={cat + i} className="px-3 py-1 rounded-full bg-neutral-900 text-white text-sm font-semibold border border-neutral-700">{cat}</span>
                ))}
              </div>
            </div>
            <div className="flex flex-col items-end">
              <div className="text-neutral-400 mb-2 text-lg">Difficulty:</div>
              <span className="px-4 py-1 rounded-full bg-lime-400 text-zinc-900 text-base font-semibold shadow-lg">{difficulty}</span>
            </div>
          </div>
          {/* Players */}
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-2 text-neutral-400">
              <Users size={20} className="text-neutral-400" /> Players ({match?.status === "WAITING" ? "1" : "2"}/2)
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-xl bg-lime-600/10 border border-lime-400 p-4 flex flex-col items-start">
                ...
                <span className="text-xs bg-neutral-900 text-lime-400 px-2 py-1 rounded">Level {user.level}</span>
              </div>
              {/* Player 2 */}
              <div className="rounded-xl bg-neutral-900 border-2 border-dashed border-neutral-700 p-4 flex flex-col items-center justify-center">
                {match?.status === "WAITING" ? (
                  <div className={`text-neutral-400 animate-blink-slow ${spaceGrotesk.className} text-center`}>Waiting for opponent...</div>
                ) : (
