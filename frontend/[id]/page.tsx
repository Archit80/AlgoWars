...
import UserService, { UserData, UserStats } from "../src/services/userService";
...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Check if idStr is a UUID (ID) or username
        const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(idStr);
        
        let data, stats;
        
        if (!isUUID) {
          // It's a username, use the username-based API endpoints
          try {
            [data, stats] = await Promise.all([
              UserService.getUserDataByUsername(idStr),
              UserService.getUserStatsByUsername(idStr)
            ]);
          } catch (userIdError) {
            console.error("Failed to fetch user with username:", userIdError);
            setError("Profile not found. The user may not exist or the username is invalid.");
            return;
          }
        } else {
          // It's a UUID, use the ID-based API endpoints
          [data, stats] = await Promise.all([
            UserService.getUserData(idStr),
            UserService.getUserStats(idStr)
          ]);
        }
        ...
        if (isCurrentUserProfile) {
          try {
            ...
            const savedRes = await res.default.getSavedQuestions(data.id); // Use the actual user ID from fetched data
  ...
  joinDate: userCreatedAt
    ...
    recentBattles: (userData.recentBattles ?? []).map((battle) => {
      ...
      let result = "draw";
      if (battle.winnerId === userId) result = "win";
      else if (battle.winnerId && battle.winnerId !== userId) result = "loss";

      // Find opponent username
      let opponent = battle.user1Id === userId ? battle.user2?.username : battle.user1?.username;
      if (!opponent) opponent = "Unknown";

      // Calculate score
      const score = battle.user1Id === userId ? battle.user1Score : battle.user2Score;
      const xp = battle.user1Id === userId ? battle.user1Score * 27 + 50 + (result === "win" ? 100 : 0) : battle.user2Score * 27 + 50 + (result === "win" ? 100 : 0);
  ...
  const profileUrl = `https://algowars.com/profile/${profileData.username}`;
